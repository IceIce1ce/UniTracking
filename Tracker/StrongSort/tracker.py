...
class Tracker:
    ...
    def __init__(self, metric, max_iou_dist=0.9, max_age=30, max_unmatched_preds=7, n_init=3, _lambda=0, ema_alpha=0.9,
                 mc_lambda=0.995):
        self.metric = metric
    ...
    def update(self, detections, confidences):
        ...
        for track_idx, detection_idx in matches:
            self.tracks[track_idx].update(detections[detection_idx], confidences[detection_idx])
    ...
    def _match(self, detections):
        def gated_metric(tracks, dets, track_indices, detection_indices):
            ...
            cost_matrix = linear_assignment.gate_cost_matrix(cost_matrix, tracks, dets, track_indices,
    ...
    def _initiate_track(self, detection, conf):
        self.tracks.append(Track(
            detection.to_xyah(), self._next_id, conf, self.n_init, self.max_age, self.ema_alpha,
            detection.feature))
        self._next_id += 1