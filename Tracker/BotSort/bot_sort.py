...
class BoTSORT(object):
    def __init__(self, model_path, track_high_thresh: float = 0.33824964456239337, new_track_thresh: float = 0.21144301345190655,
                 track_buffer: int = 60, match_thresh: float = 0.22734550911325851, proximity_thresh: float = 0.5945380911899254,
                 appearance_thresh: float = 0.4818211117541298, cmc_method: str = 'sparseOptFlow', frame_rate=30, lambda_=0.9896143462366406, use_cuda=True):
    ...
    def update(self, output_results, img):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(xyxy, s, f) for (xyxy, s, f) in zip(dets, scores_keep, features_keep)]
        else:
            detections = []
