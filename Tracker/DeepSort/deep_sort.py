...
from Tracker.DeepSort.sort.preprocessing import non_max_suppression
...
class DeepSort(object):
    def __init__(self, model_path, max_dist=0.2, min_confidence=0.3, nms_max_overlap=0.5, max_iou_distance=0.7, max_age=70, n_init=3, nn_budget=100, use_cuda=True, use_efficientdet=False):
        self.min_confidence = min_confidence
        self.nms_max_overlap = nms_max_overlap
        self.extractor = Extractor(model_path, use_cuda=use_cuda, use_extraction='wide_resnet')
    ...
    def update(self, bbox_xywh, confidences, ori_img):
        ...
        if self.use_efficientdet == True:
            features = self._get_features_efficientdet(bbox_xywh, ori_img)
        else:
