...
class CMCComputer:
    def __init__(self, minimum_features=10, method="sparse"):
        ...
        os.makedirs("Tracker/DeepOCSort/cache", exist_ok=True)
        self.cache_path = "Tracker/DeepOCSort/cache/affine_ocsort.pkl"
        self.cache = {}
        ...
        # Same BoT-SORT CMC arrays
            ...
            for f_name in os.listdir("Tracker/DeepOCSort/cache/cmc_files/MOT17_ablation/"):
                # The tag that'll be passed into compute_affine based on image name
                tag = f_name.replace("GMC-", "").replace(".txt", "") + "-FRCNN"
                f_name = os.path.join("Tracker/DeepOCSort/cache/cmc_files/MOT17_ablation/", f_name)
                self.file_names[tag] = f_name
            for f_name in os.listdir("Tracker/DeepOCSort/cache/cmc_files/MOT20_ablation/"):
                tag = f_name.replace("GMC-", "").replace(".txt", "")
                f_name = os.path.join("Tracker/DeepOCSort/cache/cmc_files/MOT20_ablation/", f_name)
                self.file_names[tag] = f_name

            # All the test file names
            for f_name in os.listdir("Tracker/DeepOCSort/cache/cmc_files/MOTChallenge/"):
                tag = f_name.replace("GMC-", "").replace(".txt", "")
                if "MOT17" in tag:
                    tag = tag + "-FRCNN"
                ...
                if tag in self.file_names:
                    continue
                f_name = os.path.join("Tracker/DeepOCSort/cache/cmc_files/MOTChallenge/", f_name)
                self.file_names[tag] = f_name
